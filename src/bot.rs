...
impl Bot {
    ...
    pub async fn update(&mut self) -> Option<IncomingMessage> {
        // Construct URL to fetch updates with timeout and current offset
        let url = format!("{}/getUpdates?timeout=30&offset={}", self.api_url, self.offset);

        // Send the request and get the raw text response
        ...
        // Loop through each update (Telegram sends an array of updates)
        for update in response_json["result"].as_array()? {
            // Advance the offset so we don't see this update again
            ...
            // Only return the message if the user is allowed
    ...
    pub async fn send_message(&self, chat_id: i64, text: &str) {
        // Send a POST request to Telegram to send a message
        let _ = self.client
